...
async def on_ready():
    ...
    while True:
        try:
            response = requests.get(f'https://api.coingecko.com/api/v3/coins/{MARKET_ID}')
            ...
            pctchng = response.json()['market_data']['price_change_percentage_24h_in_currency']['usd']
            print(f'{dt.utcnow()} | {token_name} 24hr % change: {round(pctchng,2)}%.')
            for guild in client.guilds:
                try:
                    await guild.me.edit(
                        nick=f'{token_name} ${round(price,2):,}'
                    )
                    await client.change_presence(
                        activity=Activity(
